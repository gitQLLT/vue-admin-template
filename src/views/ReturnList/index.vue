          ...
          <el-table-column type="expand">
            <template slot-scope="props">
              ...
              >
                ...
                <el-table-column prop="date" label="退货时间" />
              </el-table>
            </template>
          </el-table-column>
          <el-table-column label="序号" type="index" width="60" align="center" />
          <el-table-column label="单据编号" width="160" prop="number" />
          <el-table-column label="物流名称" width="140" prop="logisticsName" />
          <el-table-column label="物流单号" width="160" prop="logisticsInfo" />
          <el-table-column label="退料日期" width="140" prop="date">
            <template slot-scope="scope">
              {{ scope.row.date ? scope.row.date.substring(0,10) : '' }}
            </template>
          </el-table-column>
          <el-table-column label="仓库" width="140" prop="address" />
          ...
          <el-table-column label="操作" width="100">
            <template slot-scope="scope">
              <p>
                <el-link
                  :underline="false"
                  @click.stop="returnDetail(scope.$index, scope.row)"
                >详情
                </el-link>
              </p>
            </template>
...
<script>
import { returnOrder } from '@/api/procurement/return'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
import MaterialSearch from '@/components/MaterialSearch'
import { getSelected } from '@/utils/select.js'
export default {
  name: 'Return', /* 退货列表，需要与路由name一致 */
  components: {
    Pagination,
    MaterialSearch
  },
  ...
  methods: {
    ...
    getList() {
      ...
      returnOrder(this.searchQuery).then(res => {
        if (res.data) {
      ...
      fetch('https://www.fastmock.site/mock/8f5033d0d4189eefb58ba004bd6733d4/test-01/returnList')
        .then(res => res.json())
        .then(res => {
          if (res.data) {
            this.tableData = res.data.list
    ...
    returnDetail(index, row) {
      this.$router.push({ path: '/pdetail/returnDetail', query: { id: row.id }})
    },
